...
class Location(Entry):
    ...
    def pull_lines_by_fiman(self) -> dict[str, list[dict]]:
        ...
        return {fi: self.pull_lines(lns) for fi, lns in fiman_groups.items() if lns}
    ...
    def pull_ems_lines(self) -> list[dict]:
        ...
        return self.pull_lines(ems_lines) if ems_lines else []
    ...
    #             dicts_to_excel(folder / sanitize_filename(f'({len(self.lines)}) {self.building} [ALL]'),
    #               self.pull_lines())
    #
    #     return fiman_groups
